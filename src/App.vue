    ...
    <q-dialog v-model="urlDialog">
      <q-card>
        ...
        <q-card-section>
          <div class="q-mb-lg">
            <a
              href="https://startpage.com/sp/search?q=SECTION%20HEADER%20filetype:dxf"
              target="_blank"
          ...
          <q-form @submit="_OnUrl" class="q-gutter-md" style="width: 400px">
            ...
            >
              <template v-slot:hint>
                <span
                  >Uses <a href="https://allorigins.win">AllOrigins</a> CORS
                  proxy</span
                >
              </template>
            </q-input>
...
export default {
  ...
  methods: {
    async fetchFileFromServer(url) {
      try {
        ...
        var fileUrl = urlParams.get("filePath").split("/").pop();
        //const file = new File([blob], "2-6.정자1-2지구 호안전개도(준공).dxf", { type: "application/dxf" });
        const file = new File([blob], fileUrl, { type: "application/dxf" });
  ...
  created() {
    ...
    //     fileUrl = "https://rnd1.mapinus.com/disaster-rest/file/2-6.정자1-2지구 호안전개도(준공).dxf";
    // }else {
    //     fileUrl = "http://localhost:10010/disaster-rest/file/2-6.정자1-2지구 호안전개도(준공).dxf";
    // }
    var urlParams = new URLSearchParams(window.location.search);
    var fileUrl = urlParams.get("filePath");
